...
public class EditCompanyProfilePresenter {
    ...
    public void checkValidations(double latitude, double longitude, ArrayList<EditProfileInput.Interest> interestsList,
        ...
        String newCompanyName = params[2];
        String bio = params[3];
        String userImage = params[4];
        String companyLocation = params[5];
        String previousCompanyName = params[6];
        ...
        if (!TextUtils.isEmpty(companyUrl) && !Patterns.WEB_URL.matcher(companyUrl).matches()) {
            mEditProfileBaseActivity.showToast("Please enter valid company url");
            return;
        }

        editProfileInput.setCompanyUrl(companyUrl);

        if (!previousCompanyName.equals(newCompanyName)) {
            editProfileInput.setCompanyName(newCompanyName);
        }
